...
| [ADR-0023](0023-what-a-repair-corpus-measures.md) | A repair corpus measures what an agent does with a finding, not whether the finding was right: the stratum is an exact set of withholding classes because class membership overlaps, the unit is a one-or-two-line window, a repair normalizes to a break vector plus three validity judgments, the acceptable set is a verdict on a generated universe rather than a union of guesses, the bar is an absolute preregistered floor because a lower bound compared against a lower bound is not a non-inferiority test, and the elicited stimulus is a blinded hook body so the rates are a floor rather than an estimate |
...
| [ADR-0025](0025-a-split-list-item-carries-a-continuation.md) | A repair that splits a list item puts a continuation on the second line rather than a second marker, copying the continuation the file itself shows and falling back to the marker's width where it shows none; every other leader still repeats byte for byte. Two model families found this on their first batch, and it had made 19% of the drawn round unscoreable — this amends [ADR-0023](0023-what-a-repair-corpus-measures.md) |
| [ADR-0026](0026-what-fails-a-ci-build.md) | A CI run gates on rendered diagnostics rather than the checker's exit code, failing by default on `fused` alone; `wrap` fails only on opt-in and `long` in `fail-on` is refused rather than ignored |
