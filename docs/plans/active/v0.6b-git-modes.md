...
- Symlinks and gitlinks are non-checkable in every git mode:
  discovery skips them silently (they are not prose the checker owns),
  and a worktree read never follows a symlink into content git did not select.
- Unsupported repository states are loud, never silent:
  an unmerged path, a vanished index entry,
  or a `HEAD` that fails to resolve on a born branch is a `SourceError` and exit 1 —
  only a genuinely unborn branch falls back to the empty tree.
...
- Modify: `scripts/check_linefeeds.py`
  ...
  new `_segments_match`, `_exclude_match`, and `excluded` after `load_config`)
...
- Produces, in `cli/semlf/providers.py`:
  ...
  raised for git missing, git failing, an unmerged path,
  a vanished index entry, an unresolvable born `HEAD`,
  ...
  (`git diff --cached --name-status -z --diff-filter=ACMRTU`,
  which also covers an unborn HEAD),
  text read **from the index by object id**:
  the whole index is loaded once via `git ls-files --stage -z`,
  each selected path's stage-0 entry supplies `(mode, oid)`,
  and the blob is read with `git cat-file blob <oid>` —
  never by constructing a `:<path>` revision,
  which git would misparse for a legitimate filename like `0:doc.md`.
  A selected path that is unmerged (any non-zero stage) or missing from the index raises `SourceError`;
  a mode outside `{100644, 100755}` (symlink `120000`, gitlink `160000`)
  is silently skipped as non-checkable.
  `diff_sources(root)` — paths changed **worktree versus index**
  (`git diff --name-status -z --diff-filter=ACMRTU`), text read from the worktree.
  `changed_sources(root)` — paths changed **worktree versus HEAD**
  (same listing against `HEAD`;
  when `HEAD` does not resolve,
  the empty tree (`git hash-object -t tree --stdin`) substitutes **only** for a genuinely unborn branch,
  proven by `git symbolic-ref --quiet HEAD` succeeding;
  any other `HEAD` failure raises `SourceError`,
  because silently diffing against the empty tree would report every file changed),
  text read from the worktree.
  Worktree reads skip a path that is a symlink or a directory
  (`os.path.islink` / `os.path.isdir` — symlinks and gitlinks are non-checkable,
  and `open` must never follow a link into content git did not select);
  ...
  `-z` records are split on NUL and decoded with `os.fsdecode`
  (verbatim names — no `core.quotepath` mangling, spaces and non-ASCII included);
  `--name-status` records are parsed statefully
  (`R`/`C` carry two paths — the post-image name is taken;
  `U` raises `SourceError` naming the unmerged path;
  `A`/`M`/`T` carry one),
  and `--diff-filter=ACMRTU` admits typechanges (`T`) into the listing
  so the mode checks above decide them, while deletions never appear;
...
- Produces, in `tests/conftest.py`:
  ...
  `isolate_git_env(monkeypatch)` — pins `GIT_CONFIG_GLOBAL` and `GIT_CONFIG_SYSTEM` to `os.devnull`
  and clears `GIT_DIR`, `GIT_WORK_TREE`, and `GIT_INDEX_FILE`,
  so neither the helper's git nor a provider subprocess inherits host configuration, filters, hooks, or repository selectors.
  `tests/test_git_modes.py` applies it through a module-level autouse fixture.
...
def isolate_git_env(monkeypatch):
    """Pin git subprocesses against host config, env selectors, and filters."""
...
def _changed_names(root, *selector):
    ...
    --name-status rather than --name-only, because status is contract:
    U (unmerged) is a loud stop naming the path,
    R/C records carry two paths and the post-image name is the real one,
    and T (typechange) must enter the listing so the mode checks
    in the readers decide it instead of it vanishing silently.
    """
    out = _git(root, "diff", "--name-status", "-z",
               "--diff-filter=ACMRTU", *selector)
    tokens = [os.fsdecode(token) for token in out.split(b"\0")]
    names = []
    i = 0
...
def _index_entries(root):
    """{path: (mode, oid)} for stage-0 entries; an unmerged path maps to None."""
    ...
    for record in out.split(b"\0"):
        ...
        if stage != b"0":
            ...
            entries[name] = (os.fsdecode(mode), os.fsdecode(oid))
    return entries


def _head_or_empty_tree(root):
    """HEAD when it resolves; the empty tree only for a genuinely unborn branch.

    An unborn branch is a symbolic HEAD whose target ref does not exist yet.
    Any other failure (corrupt HEAD, permissions) stays loud —
    silently diffing against the empty tree would report every file changed.
    """
    try:
...
def _worktree_sources(root, names):
    ...
    for rel, display in _selected(root, names):
        ...
        if os.path.islink(full) or os.path.isdir(full):
            # Symlinks and gitlinks are non-checkable in every mode,
            # and open must never follow a link into unselected content.
            continue
        try:
...
def staged_sources(root):
    """Index versus HEAD; text is the staged blob, read by object id.

    Reading by oid rather than a ":<path>" revision keeps a filename
    like 0:doc.md from being parsed as stage syntax,
    and the mode gate keeps symlink and gitlink entries
    (non-checkable by ruling) from reaching the checker at all.
    """
    names = _changed_names(root, "--cached")
    if not names:
        return []
    ...
    for rel, display in _selected(root, names):
        entry = index.get(rel)
        if entry is None:
            state = "unmerged in" if rel in index else "missing from"
            raise SourceError(f"semlf: {display} is {state} the index")
        mode, oid = entry
...
- A recorded judgment on the opencode adapter:
  ...
  The plugin builds a Claude-shaped payload,
  so the Claude-hook CRLF subprocess test below is that adapter's coverage;
...
- Produces: ADR-0013 recording, at minimum:
  ...
  unmerged paths and an unresolvable born `HEAD` stop loudly;
  only a proven-unborn branch diffs against the empty tree);
