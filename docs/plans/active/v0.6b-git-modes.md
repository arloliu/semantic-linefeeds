...
def test_parse_raw_is_loud_for_unmerged_unknown_and_malformed():
    ...
    for stream in (meta + b"U\0conflicted.md\0",
                   ...
                   b"garbage\0a.md\0",
                   meta + b"M\0",
                   # non-octal post-image mode
                   b":100644 10z644 " + b"2" * 40 + b" " + b"3" * 40 + b" M\0a.md\0",
                   # short post-image oid
                   b":100644 100644 " + b"2" * 40 + b" 3333 M\0a.md\0",
...
def _parse_raw(out):
    ...
    the mode must be six octal digits, the oid full hexadecimal,
    ...
    while i < len(tokens) and tokens[i]:
        ...
        i += 2
        new_mode, new_oid, status = fields[1], fields[3], fields[4]
        if (len(new_mode) != 6
                or any(c not in "01234567" for c in new_mode)
                or len(new_oid) not in (40, 64)
                or any(c not in "0123456789abcdef" for c in new_oid)):
            raise SourceError("semlf: unparsable git diff record")
        if status == "U":
            raise SourceError(
