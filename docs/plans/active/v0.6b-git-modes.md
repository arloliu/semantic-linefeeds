...
- Each provider reads one snapshot:
  a single `git diff --raw -z --full-index --no-renames` enumeration carries status, post-image mode, post-image object id, and path per record,
...
- Produces, in `cli/semlf/providers.py`:
  ...
  **Enumeration is one raw listing per snapshot**:
  ...
  `git diff --raw -z --full-index --no-renames --diff-filter=AMTUX <selector>`
...
def _parse_raw(out):
    ...
    and any malformed or truncated record is a SourceError, never an IndexError.
    """
...
def _raw_records(root, *selector):
    ...
    """
    out = _git(root, "diff", "--raw", "-z", "--full-index", "--no-renames",
...
- Produces: ADR-0013 recording, at minimum:
  ...
  one `git diff --raw -z --full-index --no-renames` listing per mode carries status, post-image mode, and post-image object id together,
