...
GOOD_PAYLOAD = json.dumps(
    {"tool_input": {"file_path": "/x/a.md", "new_string": "One sentence.\n"}}
)
...
def _pipe(cmd, payload):
    ...
    try:
        proc = subprocess.run(
            cmd, input=payload.encode("utf-8"), capture_output=True, timeout=60
        )
...
def run(argv, artifact=None):
    ...
    if resolved is None:
        ...
        print(
            f"path: warn — `semlf` resolves to {resolved}, not the "
    ...
    if bad is None or good is None:
        ...
        print(
            f"replay: FAIL — fused edit expected exit 2 with a fused "
...
def _provenance_line(name, entry, dest):
    ...
    if not os.path.lexists(str(dest)):
        return (
            f"provenance: {name} warn — recorded at {_diag(entry['path'])} but missing"
        )
    try:
        mismatch = os.path.realpath(str(dest)) != os.path.realpath(entry["path"])
    except (OSError, ValueError):
        mismatch = False
    if mismatch:
        return (
            f"provenance: {name} warn — recorded for {_diag(entry['path'])}, "
            f"expected {dest}"
        )
...
def _codex_hook_check():
    ...
    if data is None:
        print(
            "codex hook: FAIL — hooks.json is unreadable, oversized, "
    ...
    if os.name == "nt":
        ...
        print(
            "codex hook: FAIL — installed, but replay is not yet "
    ...
    for argv in owned:
        try:
            if not stat.S_ISREG(os.lstat(argv[1]).st_mode):
                print(
                    f"codex hook: FAIL — configured checker is not a "
        ...
        if fused[0] != 2 or "fused" not in fused[1]:
            print(
                f"codex hook: FAIL — fused patch expected exit 2 with a "
        ...
        if clean[0] != 0:
            ...
            return 1
    print(
        f"codex hook: ok — {len(owned)} owned entr"
